class PlayFairCipher:
    ...
    def playfair_decrypt(self, cipher_text, matrix):
        ...
        # Loại bỏ ký tự 'X' chèn giữa hai ký tự giống nhau hoặc ở cuối
        plain_text = ""
        i = 0
        while i < len(decrypted_text):
            plain_text += decrypted_text[i]
            # Nếu ký tự tiếp theo là 'X' và ký tự sau 'X' giống ký tự hiện tại, bỏ 'X'
            if (i+2 < len(decrypted_text) and decrypted_text[i+1] == 'X' and decrypted_text[i] == decrypted_text[i+2]):
                i += 2
            else:
                i += 1
        # Nếu ký tự cuối là 'X', loại bỏ
